...
def load_lib(fullname):
    """ try to load from ldraw dir, then data_dir """
    ldraw_path = os.path.abspath(os.path.dirname(__file__))
    data_dir = get_data_dir()
    try:
        return load_lib_from(fullname, ldraw_path)
    except ImportError:
        return load_lib_from(fullname, data_dir)
